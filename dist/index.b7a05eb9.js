...
console.log(//toArray<string>('neo', 123), //오버로드와 같이 제네릭 문법에서도 첫번째 인자로 인하여 이후의 인자들의 타입도 정해지게 됨 (string, number) 불가
toArray("neo", "123"), toArray(1, 2), toArray(true, false), toArray({
    x: 1
}, {
    y: 2
}), toArray([
    ...
    3,
    4
]) // =>의도는 tuple type으로 [a, b] 각각의 값이 숫자이며 2개인 배열 but ([1, 2, 3])에서도 오류를 발생시키지 않음(다른 예과 같이 추론으로는 안 됨) generic의 타입을 명시하지 않았기에 타입추론은 일반 배열 타입으로 인식하게 됨
);
//제네릭
//클래스
// class User {
...
console.log((0, _lodashDefault.default).kebabCase(str)) // 이처럼 js 파일은 dts파일을 통해 불러와야 함
;
